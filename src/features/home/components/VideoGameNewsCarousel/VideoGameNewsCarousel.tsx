...
import { useGetNewsVideoGamesQuery } from '../../redux/homeApiSlice';
...
const VideoGameNewsCarousel = ({
  navigationPrevClassname,
  navigationNextClassname,
  paginationClassname,
}: VideoGamesCarouselProps) => {

  const { data: videoGamesNews, isLoading, isError } = useGetNewsVideoGamesQuery();
  ...
  if (isError || isLoading || !isLoading && !videoGamesNews) return <></>
  ...
  return (
    <>
      ...
      >
        {videoGamesNews?.map((data, index: number) =>
...
export default VideoGameNewsCarousel;